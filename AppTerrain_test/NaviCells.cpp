...
void CNaviCells::Initialize( CTriContainer &triContainer )
{
...
void CNaviCells::Release()
{
	m_vecNavicells.clear();
}

void CNaviCells::buildupAdjacentAll( VEC_NAVICELL &vecNaviCells )
{
...
bool CNaviCells::checkAllreadyAdj(	CNaviCell &cellCheck )
{
...
void CNaviCells::setAdjacentATri(	CNaviCell &cellLeft,
...
//	E_CELL_SIDEEDGE	eCellsSideEdge;
	...
	for( it_=iterBegin; it_!=iterEnd; ++it_ )
	{
		...
		if(NULL==ptriRight)
		{
			//assert(0 && "NULL==ptriRight in CNaviCells::setAdjacentATri().");
			assert__(ptriRight,__FILE__,__LINE__,__FUNCTION__,"NULL==ptriRight in CNaviCells::setAdjacentATri().");
		}
		// For TESTDEBUG
#endif // _DEBUG
		...
		if( true==bFoundShared )
		{
			...
			if(iFoundShared>2)
			{
				assert__((iFoundShared<3),__FILE__,__LINE__,__FUNCTION__,"iFoundShared>2 in CNaviCells::setAdjacentATri().");
				break;
			}

			if(iSharedIV_left==iSharedIV_right)
			{
				assert__(iSharedIV_left!=iSharedIV_right,__FILE__,__LINE__,__FUNCTION__,"iFoundShared>2 in CNaviCells::setAdjacentATri().");
				break;
			}
...
E_CELL_SIDEEDGE CNaviCells::getTypeEdge(	const unsigned int uiSharedNum )
{
...
// Non Recursive
void CNaviCells::mappingNavigation__( VEC_NAVICELL &vecNaviCells, set<int> &slistCellsGoal )
{
...
void CNaviCells::mappingNavigationRecurse(	VEC_NAVICELL &vecNaviCells_, 
											set<int> &slistCells_,
...
bool CNaviCells::constructMappingNavigation( set<int> &slistCellsGoal, set<int> &slistCellsBlock )
{
...
bool CNaviCells::getCell(	const unsigned int iIdxNavicell, 
							CNaviCell &naviCell)
{
...
void CNaviCells::ConstructTrisToCells( CTriContainer &triContainer )
{
...
void CNaviCells::setClearAllcell_unprocessed(	VEC_NAVICELL &vecNaviCells )
{
	VEC_NAVICELL::iterator	it_, iterBegin, iterEnd;
	iterBegin = vecNaviCells.begin(); iterEnd = vecNaviCells.end();
	for( it_=iterBegin; it_!=iterEnd; ++it_ )
...
// For TESTDEBUG
void CNaviCells::validateAllAdjcntCells(	VEC_NAVICELL &vecNavicells )
{
...
void CNaviCells::validateAdjcntRecurse(	int iSeqCell, VEC_NAVICELL &vecNavicells )
{
